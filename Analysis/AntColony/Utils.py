...
class ComponentGraphBuilder:
    ...
    def compute_components_graph(
        self, layers_assignments: dict[str, int], model_graph: nx.DiGraph
    ) -> nx.DiGraph:
        start = time.perf_counter_ns()
        nodes_components: dict[str, tuple[int, int]] = self.find_nodes_components(
            layers_assignments, model_graph
        )
            ...
            if first_comp_id != second_comp_id:
                if (first_comp_id, second_comp_id) not in component_graph.edges:
                    component_graph.add_edge(
                        first_comp_id, second_comp_id, tensors=set()
                    )

                edge_tensors = model_graph.edges[layer_edge]["tensor_name_list"]
                component_graph.edges[first_comp_id, second_comp_id]["tensors"].update(
                    edge_tensors
                )
...
def draw_parallel_component_graph(component_graph: nx.DiGraph, filename=None):
    ...
    for node in nx.topological_sort(component_graph):
        if component_graph.in_degree(node) == 0:
            ...
            levels[node] = 1 + max(
                levels[pred] for pred in component_graph.predecessors(node)
            )
    ...
    # Draw labels
    labels = {
        node: f"{node[0]}:{node[1]}" for node in component_graph.nodes
    }  # e.g., server:comp_idx
    nx.draw_networkx_labels(
        component_graph, pos, labels=labels, font_size=10, font_weight="bold"
    )
